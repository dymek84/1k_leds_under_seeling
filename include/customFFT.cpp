...
customFFT::customFFT(float *vReal, float *vImag, uint16_t samples, float samplingFrequency)
{ // Constructor
    this->_vReal = vReal;
    this->_vImag = vImag;
...
customFFT::~customFFT(void)
{
    // Destructor
}

uint8_t customFFT::Revision(void)
{
    return (FFT_LIB_REV);
}

void customFFT::Compute(uint8_t dir)
...
void customFFT::ComplexToMagnitude()
...
void customFFT::Windowing(uint8_t windowType, uint8_t dir)
{ // Weighing factors are computed once before multiple use of FFT
    ...
    for (uint16_t i = 0; i < (this->_samples >> 1); i++)
    {
        ...
        case FFT_WIN_TYP_NUTTALL: // nuttall
            weighingFactor = 0.355768 - (0.487396 * (cos(twoPi * ratio))) + (0.144232 * (cos(fourPi * ratio))) -
        ...
        case FFT_WIN_TYP_BLACKMAN_NUTTALL: // blackman nuttall
            weighingFactor = 0.3635819 - (0.4891775 * (cos(twoPi * ratio))) + (0.1365995 * (cos(fourPi * ratio))) -
                             (0.0106411 * (cos(sixPi * ratio)));
            break;
        case FFT_WIN_TYP_BLACKMAN_HARRIS: // blackman harris
            weighingFactor = 0.35875 - (0.48829 * (cos(twoPi * ratio))) + (0.14128 * (cos(fourPi * ratio))) -
...
float customFFT::MajorPeak()
{
    ...
    float delta =
        0.5 * ((this->_vReal[IndexOfMaxY - 1] - this->_vReal[IndexOfMaxY + 1]) /
               (this->_vReal[IndexOfMaxY - 1] - (2.0 * this->_vReal[IndexOfMaxY]) + this->_vReal[IndexOfMaxY + 1]));
...
void customFFT::MajorPeak(float *f, float *v)
{
    float maxY = 0;
    uint16_t IndexOfMaxY = 0;
    // If sampling_frequency = 2 * max_frequency in signal,
...
uint8_t customFFT::Exponent(uint16_t value)
{
...
void customFFT::Swap(float *x, float *y)
{
    float temp = *x;
    *x = *y;
    *y = temp;
